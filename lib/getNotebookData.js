...
module.exports = ({ config, dom }) => {
  const { nodeIsIgnored, nodeIsCode, getRootNode, getCodeCells } = config;
      ...
      if (nodeHasChildren(node) && cellType === CELL_TYPE_TEXT) {
        ...
        switch (cellType) {
          case CELL_TYPE_CODE:
            ...
            break;
          default:
            newCells = [{ type: CELL_TYPE_TEXT, text: getText(node) }];
            break;
        }

        newCells.forEach((cell) => {
          // Start new cell if cell type has changed or on every code cell
          if (hasCellTypeChanged || cellType === CELL_TYPE_CODE) cells.push({ ...cell, text: '' });
